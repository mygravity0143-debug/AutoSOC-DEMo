...
const messageFrequency = new Map<string, { count: number; windowStart: number }>();
const diagnosticRequests = new Map<string, number[]>();
const floodDetection = new Map<string, number[]>();

function trackFrequency(key: string, windowMs: number = 10000): number {
  const now = Date.now();
...
function trackFlood(vehicleId: string): number {
  const now = Date.now();
...
function trackDiagnosticRequests(vehicleId: string): number {
  const now = Date.now();
...
export function analyzeCANMessage(event: {
  vehicle_id: string;
  ecu_id: string;
  can_id: string;
  dlc: number;
...
export function processSimulationEvent(
  ...
  const templates: Record<string, DetectionResult> = {
    'unknown-can-id': {
      detected: true,
      threatType: 'UNKNOWN_CAN_ID',
      severity: 'HIGH',
