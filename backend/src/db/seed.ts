...
export async function seedDatabase() {
  const existing = await getOne<{ count: number }>('SELECT COUNT(*) as count FROM vehicles');
  ...
  for (const v of vehicles) {
    await runQuery(
      `INSERT INTO vehicles (id, vin, model, software_version, ecu_count, can_interface, risk_score, security_status, last_seen)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [v.id, v.vin, v.model, v.software_version, v.ecu_count, v.can_interface, v.risk_score, v.security_status, now]
    );
    ...
    for (let i = 0; i < ecus.length; i++) {
      ...
      await runQuery(
        `INSERT INTO ecus (id, vehicle_id, name, type, status, firmware_version, message_rate, security_status, last_event)
         VALUES (?, ?, ?, ?, 'ACTIVE', ?, ?, ?, ?)`,
        [ecuId, v.id, ecu.name, ecu.type, ecu.fw, ecu.rate, secStatus, now]
      );
    }
  }
  ...
  for (const vuln of vulnerabilities) {
    await runQuery(
      `INSERT INTO vulnerabilities (id, ecu_name, vehicle_id, title, description, severity, cvss_score, status, remediation)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [vuln.id, vuln.ecu_name, vuln.vehicle_id, vuln.title, vuln.description, vuln.severity, vuln.cvss_score, vuln.status, vuln.remediation]
    );
  }
  ...
  await runQuery(
    `INSERT INTO notifications (id, timestamp, title, message, severity, read)
     VALUES (?, ?, ?, ?, ?, 0)`,
    ['NOTIF-1', now, 'System Online', 'AutoSOC Detection Engine initialized and monitoring 10 vehicles.', 'LOW']
  );
